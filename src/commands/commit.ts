...
function checkMessageTemplate(extraArgs: string[]): string | false {
   for (const key in extraArgs) {
      if (extraArgs[key].includes(config?.MOJI_MESSAGE_TEMPLATE_PLACEHOLDER))
         return extraArgs[key]
   }
   return false
}

async function generateCommitMessageFromGitDiff(diff: string, extraArgs: string[]): Promise<void> {
   await assertGitRepo()
