...
async function generateCommitMessageChatCompletionPrompt(diff: string): Promise<Array<ChatCompletionRequestMessage>> {
...
export async function generateCommitMessageByDiff(diff: string): Promise<string> {
   try {
      const INIT_MESSAGES_PROMPT = await getMainCommitPrompt()
...
export async function getCommitMsgsPromisesFromFileDiffs(diff: string, maxDiffLength: number) {
