...
async function run() {
   intro('ConmitMoji — improving lame commit messages')
   ...
   try {
      if (github.context.eventName === 'push') {
         ...
         outro('Wrong action.')
         core.error(
        `ConmitMoji was called on ${github.context.payload.action}. ConmitMoji is supposed to be used on "push" action.`,
         )
      }
   }
   catch (error: any) {
